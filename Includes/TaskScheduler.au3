...
; #FUNCTION# ====================================================================================================================
; Name ..........: _TS_ErrorText
; Description ...: Returns the full error message text for an error number.
...
;                  $bPrefix      - Optional: True prefixes the error with functionname and errornumber (default = True)
...
Func _TS_ErrorText($iErrorNumber, $bPrefix = True)
	...
	For $i = 0 To UBound($aErrorMessages, 1) - 1
		If $aErrorMessages[$i][1] = $iErrorNumber Then
			If $bPrefix Then Return $aErrorMessages[$i][0] & "(" & $aErrorMessages[$i][1] & "): " & $aErrorMessages[$i][2]
...
Func _TS_VersionInfo()
	Local $aVersionInfo[9] = [8, "V", 1, 5, 0.0, "20191127", "3.3.14.5", "allow2010, AdmUL, water", ""]
	Return $aVersionInfo
...
Func __TS_ConvertDaysOfMonth($BitValue)
	Local $sMsg = ""
	If BitAND($BitValue, 0x01) Then $sMsg = "1"
	If BitAND($BitValue, 0x02) Then $sMsg = $sMsg & "2"
	If BitAND($BitValue, 0x04) Then $sMsg = $sMsg & "3"
	If BitAND($BitValue, 0x08) Then $sMsg = $sMsg & "4"
	If BitAND($BitValue, 0x10) Then $sMsg = $sMsg & "5"
	If BitAND($BitValue, 0x20) Then $sMsg = $sMsg & "6"
	If BitAND($BitValue, 0x40) Then $sMsg = $sMsg & "7"
	If BitAND($BitValue, 0x80) Then $sMsg = $sMsg & "8"
	If BitAND($BitValue, 0x100) Then $sMsg = $sMsg & "9"
	If BitAND($BitValue, 0x200) Then $sMsg = $sMsg & "10"
	If BitAND($BitValue, 0x400) Then $sMsg = $sMsg & "11"
	If BitAND($BitValue, 0x800) Then $sMsg = $sMsg & "12"
	If BitAND($BitValue, 0x1000) Then $sMsg = $sMsg & "13"
	If BitAND($BitValue, 0x2000) Then $sMsg = $sMsg & "14"
	If BitAND($BitValue, 0x4000) Then $sMsg = $sMsg & "15"
	If BitAND($BitValue, 0x8000) Then $sMsg = $sMsg & "16"
	If BitAND($BitValue, 0x10000) Then $sMsg = $sMsg & "17"
	If BitAND($BitValue, 0x20000) Then $sMsg = $sMsg & "18"
	If BitAND($BitValue, 0x40000) Then $sMsg = $sMsg & "19"
	If BitAND($BitValue, 0x80000) Then $sMsg = $sMsg & "20"
	If BitAND($BitValue, 0x100000) Then $sMsg = $sMsg & "21"
	If BitAND($BitValue, 0x200000) Then $sMsg = $sMsg & "22"
	If BitAND($BitValue, 0x400000) Then $sMsg = $sMsg & "23"
	If BitAND($BitValue, 0x800000) Then $sMsg = $sMsg & "24"
	If BitAND($BitValue, 0x1000000) Then $sMsg = $sMsg & "25"
	If BitAND($BitValue, 0x2000000) Then $sMsg = $sMsg & "26"
	If BitAND($BitValue, 0x4000000) Then $sMsg = $sMsg & "27"
	If BitAND($BitValue, 0x8000000) Then $sMsg = $sMsg & "28"
	If BitAND($BitValue, 0x10000000) Then $sMsg = $sMsg & "29"
	If BitAND($BitValue, 0x20000000) Then $sMsg = $sMsg & "30"
	If BitAND($BitValue, 0x40000000) Then $sMsg = $sMsg & "31"
	If BitAND($BitValue, 0x80000000) Then $sMsg = $sMsg & "LAST"
	If StringLeft($sMsg, 2) = ", " Then $sMsg = StringMid($sMsg, 3)     ; Remove leading ", "
	Return $sMsg
EndFunc   ;==>__TS_ConvertDaysOfMonth

Func __TS_ConvertDaysOfWeek($BitValue)
	Local $sMsg = ""
	If BitAND($BitValue, 1) Then $sMsg = "Sunday"
	If BitAND($BitValue, 2) Then $sMsg = $sMsg & ", Monday"
	If BitAND($BitValue, 4) Then $sMsg = $sMsg & ", Tuesday"
	If BitAND($BitValue, 8) Then $sMsg = $sMsg & ", Wednesday"
	If BitAND($BitValue, 16) Then $sMsg = $sMsg & ", Thursday"
	If BitAND($BitValue, 32) Then $sMsg = $sMsg & ", Friday"
	If BitAND($BitValue, 64) Then $sMsg = $sMsg & ", Saturday"
	If StringLeft($sMsg, 2) = ", " Then $sMsg = StringMid($sMsg, 3)     ; Remove leading ", "
	Return $sMsg
EndFunc   ;==>__TS_ConvertDaysOfWeek

Func __TS_ConvertMonthsOfYear($BitValue)
	Local $sMsg = ""
	If BitAND($BitValue, 1) Then $sMsg = "January"
	If BitAND($BitValue, 2) Then $sMsg = $sMsg & ", February"
	If BitAND($BitValue, 4) Then $sMsg = $sMsg & ", March"
	If BitAND($BitValue, 8) Then $sMsg = $sMsg & ", April"
	If BitAND($BitValue, 16) Then $sMsg = $sMsg & ", May"
	If BitAND($BitValue, 32) Then $sMsg = $sMsg & ", June"
	If BitAND($BitValue, 64) Then $sMsg = $sMsg & ", July"
	If BitAND($BitValue, 128) Then $sMsg = $sMsg & ", August"
	If BitAND($BitValue, 256) Then $sMsg = $sMsg & ", September"
	If BitAND($BitValue, 512) Then $sMsg = $sMsg & ", October"
	If BitAND($BitValue, 1024) Then $sMsg = $sMsg & ", November"
	If BitAND($BitValue, 2048) Then $sMsg = $sMsg & ", December"
	If StringLeft($sMsg, 2) = ", " Then $sMsg = StringMid($sMsg, 3)     ; Remove leading ", "
	Return $sMsg
EndFunc   ;==>__TS_ConvertMonthsOfYear

Func __TS_ConvertWeeksOfMonth($BitValue)
	Local $sMsg = ""
	If BitAND($BitValue, 1) Then $sMsg = "First"
	If BitAND($BitValue, 2) Then $sMsg = $sMsg & ", Second"
	If BitAND($BitValue, 4) Then $sMsg = $sMsg & ", Third"
	If BitAND($BitValue, 8) Then $sMsg = $sMsg & ", Fourth"
	If StringLeft($sMsg, 2) = ", " Then $sMsg = StringMid($sMsg, 3)     ; Remove leading ", "
	Return $sMsg
